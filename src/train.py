...
def train(model, **kargs):
    model.train()
    output = model(x=Variable(torch.FloatTensor(kargs['train'])).to(kargs['device']))
...
def eval(model, **kargs):
    ...
    for _ in range(len(kargs['dev'])):
        output = model.forward(x=Variable(torch.FloatTensor(kargs['dev'][_])).to(kargs['device']))
...
def predict(model, **kargs):
    ...
    for _ in range(len(kargs['test'])):
        output = model.forward(x=Variable(torch.FloatTensor(kargs['test'][_])).to(kargs['device'])).detach()
