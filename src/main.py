...
#     parser.add_argument('-per-gpu-eval-batch-size', type=int, default=32, help='per gpu batch size for eval [default: 32]')
    ...
    parser.add_argument('-kernel-size', type=int, default=3, help='cnn kernel size')
    parser.add_argument('-input-channel', type=int, default=1, help='cnn input channel')
    parser.add_argument('-seq-len', type=int, default=400, help='input seq len')
    parser.add_argument('-kernel-num', type=int, default=768, help='cnn kernel num')
    ...
    if args.do_train:
        ...
        for i, step in enumerate(bar):
            ...
            # train
            train_loss = train.train(cnn, train=batch_train, train_label=batch_label, loss_func=loss_func, optimizer=optimizer, device=device, eval_result=args.eval_result, step=i)
            bar.set_description("loss {}".format(train_loss))
            
            # eval
            macro_f1 = train.eval(cnn, dev=dev, dev_label=dev_label, batch_size=args.batch_size, device=device, num_label=args.class_num, eval_result=args.eval_result, step=i)
    ...
    if args.do_predict:
        ...
        train.predict(cnn, test=test, device=device, file=args.predict_file)
        print('predict saved')
        print('='*80)
    bc.close()    